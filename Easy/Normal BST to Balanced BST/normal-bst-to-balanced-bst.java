...
class GfG
{
    Node buildBalancedTree(Node root) 
    {
        ...
        Node head = buildTree(lst, 0, lst.size() - 1);
        return head;
    }
     public void inorderTraversal(Node root, List<Integer> lst) {
        if (root == null) {
            return;
        }
    ...
    public Node buildTree(List<Integer> lst, int low, int high) {
        if (low > high) return null;
        int mid=(low+high)/2;
        Node root=new Node(lst.get(mid));
